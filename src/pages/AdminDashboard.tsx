...
import { Users, DollarSign, GraduationCap, CheckCircle, XCircle, Search, LogOut, Home, UserCheck, BarChart3, FileText, Download, ArrowLeft, MessageSquare } from "lucide-react";
import type { Student } from "@/data/mockData";

const AdminDashboard = () => {
  const { user, logout, students, pendingUsers, approveUser, rejectUser, addRemark } = useAuth();
  const navigate = useNavigate();
  const { toast } = useToast();
  const [activeTab, setActiveTab] = useState<"overview" | "students" | "pending" | "analytics">("overview");
  ...
  const filteredStudents = students.filter((s) => {
    const matchSearch = s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.course.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleAddRemark = () => {
    ...
    addRemark(selectedStudent.id, remark);
      ...
      <div className="min-h-screen bg-background">
        <header className="bg-primary text-primary-foreground h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-50">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display font-semibold text-lg hidden sm:block">SCHUPA Admin</span>
          </div>
